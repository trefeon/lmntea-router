...
/**
 * Generated by scripts/import-provider.ts for provider "openai"
 * Sample model: openai/gpt-5.6
 * Citations: reference/OmniRoute/src/shared/constants/modelSpecs.ts:111 (spread GPT_5_6_MODEL_SPEC from reference/OmniRoute/src/shared/constants/modelSpecs.ts:90 (GPT_5_6_MODEL_SPEC)) | universal_protocol_translation_spec.md:effort table (thinking caps)
 * Provider citations: reference/OmniRoute/open-sse/config/providers/registry/openai/index.ts:5 | reference/OmniRoute/src/shared/constants/modelSpecs.ts:111 (spread GPT_5_6_MODEL_SPEC from reference/OmniRoute/src/shared/constants/modelSpecs.ts:90 (GPT_5_6_MODEL_SPEC))
 */

describe('import-provider generated: openai', () => {
  const sampleId = 'openai/gpt-5.6'
  const spec = MODEL_REGISTRY[sampleId]

  it('provider exists in PROVIDERS with baseUrl', () => {
    const prov = PROVIDERS['openai']
    expect(prov).toBeDefined()
    expect(prov.baseUrl).toMatch(/^https?:\/\//)
    // citation: reference/OmniRoute/open-sse/config/providers/registry/openai/index.ts:5 | reference/OmniRoute/src/shared/constants/modelSpecs.ts:111 (spread GPT_5_6_MODEL_SPEC from reference/OmniRoute/src/shared/constants/modelSpecs.ts:90 (GPT_5_6_MODEL_SPEC))
  })
  ...
  it('model exists in MODEL_REGISTRY with sorted citations', () => {
    ...
    expect(spec.contextWindow).toBe(1050000)
  ...
  it('vision gate: supportsVision citation preserved', () => {
    if (!spec) return
    // visionGate=true from source reference/OmniRoute/src/shared/constants/modelSpecs.ts:111 (spread GPT_5_6_MODEL_SPEC from reference/OmniRoute/src/shared/constants/modelSpecs.ts:90 (GPT_5_6_MODEL_SPEC)) | universal_protocol_translation_spec.md:effort table (thinking caps)
    expect(spec).toBeDefined()
  })
  ...
  it('relay vs direct tier: provider relay flag coherent', () => {
    const prov = PROVIDERS['openai']
    // relay=false passthrough=false citation: reference/OmniRoute/open-sse/config/providers/registry/openai/index.ts:5 | reference/OmniRoute/src/shared/constants/modelSpecs.ts:111 (spread GPT_5_6_MODEL_SPEC from reference/OmniRoute/src/shared/constants/modelSpecs.ts:90 (GPT_5_6_MODEL_SPEC))
  ...
  it('app.request integration: POST /v1/chat/completions normalizes via clamp/sanitize/thinking', async () => {
    ...
    if (res.status === 501) {
      ...
      if (normalized) expect(Number(normalized)).toBeLessThanOrEqual(128000)
    }
  })
})
