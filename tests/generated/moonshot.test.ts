...
/**
 * Generated by scripts/import-provider.ts for provider "moonshot"
 * Sample model: moonshot/kimi-k3
 * Citations: reference/OmniRoute/src/shared/constants/modelSpecs.ts:437 | universal_protocol_translation_spec.md:effort table (thinking caps)
 * Provider citations: reference/OmniRoute/open-sse/config/providers/registry/moonshot/index.ts:6 | reference/OmniRoute/src/shared/constants/modelSpecs.ts:437
 */

describe('import-provider generated: moonshot', () => {
  const sampleId = 'moonshot/kimi-k3'
  ...
  it('provider exists in PROVIDERS with baseUrl', () => {
    ...
    // citation: reference/OmniRoute/open-sse/config/providers/registry/moonshot/index.ts:6 | reference/OmniRoute/src/shared/constants/modelSpecs.ts:437
  })

  it('model exists in MODEL_REGISTRY with sorted citations', () => {
    expect(spec).toBeDefined()
    expect(spec.id).toBe(sampleId)
    expect(spec.provider).toBe('moonshot')
    expect(spec.contextWindow).toBe(1048576)
    expect(spec.maxOutputTokens).toBe(1048576)
  })
  ...
  it('vision gate: supportsVision citation preserved', () => {
    if (!spec) return
    // visionGate=true from source reference/OmniRoute/src/shared/constants/modelSpecs.ts:437 | universal_protocol_translation_spec.md:effort table (thinking caps)
    expect(spec).toBeDefined()
  })
  ...
  it('relay vs direct tier: provider relay flag coherent', () => {
    const prov = PROVIDERS['moonshot']
    // relay=false passthrough=false citation: reference/OmniRoute/open-sse/config/providers/registry/moonshot/index.ts:6 | reference/OmniRoute/src/shared/constants/modelSpecs.ts:437
  ...
  it('app.request integration: POST /v1/chat/completions normalizes via clamp/sanitize/thinking', async () => {
    ...
    if (res.status === 501) {
      ...
      if (normalized) expect(Number(normalized)).toBeLessThanOrEqual(1048576)
    }
  })
})
