...
/**
 * Generated by scripts/import-provider.ts for provider "cohere"
 * Sample model: cohere/command-r-plus-08-2024
 * Citations: reference/9router/open-sse/providers/registry/cohere.js:2 | reference/9router/open-sse/providers/registry (baseUrl/format) | https://openrouter.ai/api/v1/models (context_length=128000, top_provider.max_completion_tokens=4000)
 * Provider citations: reference/9router/open-sse/providers/registry/cohere.js:2
 */

describe('import-provider generated: cohere', () => {
  const sampleId = 'cohere/command-r-plus-08-2024'
  const spec = MODEL_REGISTRY[sampleId]

  it('provider exists in PROVIDERS with baseUrl', () => {
    const prov = PROVIDERS['cohere']
    expect(prov).toBeDefined()
    expect(prov.baseUrl).toMatch(/^https?:\/\//)
    // citation: reference/9router/open-sse/providers/registry/cohere.js:2
  })

  it('model exists in MODEL_REGISTRY with sorted citations', () => {
    expect(spec).toBeDefined()
    expect(spec.id).toBe(sampleId)
    expect(spec.provider).toBe('cohere')
    expect(spec.contextWindow).toBe(128000)
    expect(spec.maxOutputTokens).toBe(4000)
  })
  ...
  it('vision gate: supportsVision citation preserved', () => {
    if (!spec) return
    // visionGate=unknown from source reference/9router/open-sse/providers/registry/cohere.js:2 | reference/9router/open-sse/providers/registry (baseUrl/format) | https://openrouter.ai/api/v1/models (context_length=128000, top_provider.max_completion_tokens=4000)
    expect(spec).toBeDefined()
  })
  ...
  it('relay vs direct tier: provider relay flag coherent', () => {
    const prov = PROVIDERS['cohere']
    // relay=false passthrough=false citation: reference/9router/open-sse/providers/registry/cohere.js:2
  ...
  it('app.request integration: POST /v1/chat/completions normalizes via clamp/sanitize/thinking', async () => {
    ...
    if (res.status === 501) {
      ...
      if (normalized) expect(Number(normalized)).toBeLessThanOrEqual(4000)
    }
  })
})
