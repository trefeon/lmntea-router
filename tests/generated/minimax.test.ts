...
/**
 * Generated by scripts/import-provider.ts for provider "minimax"
 * Sample model: minimax/minimax-m3
 * Citations: reference/OmniRoute/src/shared/constants/modelSpecs.ts:619 | universal_protocol_translation_spec.md:effort table (thinking caps)
 * Provider citations: reference/OmniRoute/open-sse/config/providers/registry/minimax/index.ts:4 | reference/OmniRoute/src/shared/constants/modelSpecs.ts:619
 */

describe('import-provider generated: minimax', () => {
  const sampleId = 'minimax/minimax-m3'
  const spec = MODEL_REGISTRY[sampleId]

  it('provider exists in PROVIDERS with baseUrl', () => {
    const prov = PROVIDERS['minimax']
    expect(prov).toBeDefined()
    expect(prov.baseUrl).toMatch(/^https?:\/\//)
    // citation: reference/OmniRoute/open-sse/config/providers/registry/minimax/index.ts:4 | reference/OmniRoute/src/shared/constants/modelSpecs.ts:619
  })
  ...
  it('model exists in MODEL_REGISTRY with sorted citations', () => {
    ...
    expect(spec.maxOutputTokens).toBe(512000)
  })
  ...
  it('vision gate: supportsVision citation preserved', () => {
    if (!spec) return
    // visionGate=unknown from source reference/OmniRoute/src/shared/constants/modelSpecs.ts:619 | universal_protocol_translation_spec.md:effort table (thinking caps)
    expect(spec).toBeDefined()
  })
  ...
  it('relay vs direct tier: provider relay flag coherent', () => {
    const prov = PROVIDERS['minimax']
    // relay=false passthrough=false citation: reference/OmniRoute/open-sse/config/providers/registry/minimax/index.ts:4 | reference/OmniRoute/src/shared/constants/modelSpecs.ts:619
  ...
  it('app.request integration: POST /v1/chat/completions normalizes via clamp/sanitize/thinking', async () => {
    ...
    if (res.status === 501) {
      ...
      if (normalized) expect(Number(normalized)).toBeLessThanOrEqual(512000)
    }
  })
})
