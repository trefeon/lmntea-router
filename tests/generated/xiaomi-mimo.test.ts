...
/**
 * Generated by scripts/import-provider.ts for provider "xiaomi-mimo"
 * Sample model: xiaomi-mimo/mimo-v2.5-pro
 * Citations: reference/OmniRoute/src/shared/constants/modelSpecs.ts:523 | universal_protocol_translation_spec.md:effort table (thinking caps)
 * Provider citations: reference/OmniRoute/open-sse/config/providers/registry/xiaomi-mimo/index.ts:5 | reference/OmniRoute/src/shared/constants/modelSpecs.ts:523
 */

describe('import-provider generated: xiaomi-mimo', () => {
  const sampleId = 'xiaomi-mimo/mimo-v2.5-pro'
  const spec = MODEL_REGISTRY[sampleId]

  it('provider exists in PROVIDERS with baseUrl', () => {
    const prov = PROVIDERS['xiaomi-mimo']
    expect(prov).toBeDefined()
    expect(prov.baseUrl).toMatch(/^https?:\/\//)
    // citation: reference/OmniRoute/open-sse/config/providers/registry/xiaomi-mimo/index.ts:5 | reference/OmniRoute/src/shared/constants/modelSpecs.ts:523
  })

  it('model exists in MODEL_REGISTRY with sorted citations', () => {
    expect(spec).toBeDefined()
    expect(spec.id).toBe(sampleId)
    expect(spec.provider).toBe('xiaomi-mimo')
  ...
  it('vision gate: supportsVision citation preserved', () => {
    if (!spec) return
    // visionGate=false from source reference/OmniRoute/src/shared/constants/modelSpecs.ts:523 | universal_protocol_translation_spec.md:effort table (thinking caps)
    expect(spec).toBeDefined()
  })
  ...
  it('relay vs direct tier: provider relay flag coherent', () => {
    const prov = PROVIDERS['xiaomi-mimo']
    // relay=false passthrough=false citation: reference/OmniRoute/open-sse/config/providers/registry/xiaomi-mimo/index.ts:5 | reference/OmniRoute/src/shared/constants/modelSpecs.ts:523
