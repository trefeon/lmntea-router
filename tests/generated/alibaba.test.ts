...
/**
 * Generated by scripts/import-provider.ts for provider "alibaba"
 * Sample model: alibaba/qwen3-max
 * Citations: reference/OmniRoute/src/shared/constants/modelSpecs.ts:483 | universal_protocol_translation_spec.md:effort table (thinking caps)
 * Provider citations: reference/OmniRoute/open-sse/config/providers/registry/alibaba/index.ts:4 | reference/OmniRoute/src/shared/constants/modelSpecs.ts:483
 */

describe('import-provider generated: alibaba', () => {
  const sampleId = 'alibaba/qwen3-max'
  const spec = MODEL_REGISTRY[sampleId]

  it('provider exists in PROVIDERS with baseUrl', () => {
    const prov = PROVIDERS['alibaba']
    expect(prov).toBeDefined()
    expect(prov.baseUrl).toMatch(/^https?:\/\//)
    // citation: reference/OmniRoute/open-sse/config/providers/registry/alibaba/index.ts:4 | reference/OmniRoute/src/shared/constants/modelSpecs.ts:483
  })

  it('model exists in MODEL_REGISTRY with sorted citations', () => {
    expect(spec).toBeDefined()
    expect(spec.id).toBe(sampleId)
    expect(spec.provider).toBe('alibaba')
  ...
  it('vision gate: supportsVision citation preserved', () => {
    if (!spec) return
    // visionGate=true from source reference/OmniRoute/src/shared/constants/modelSpecs.ts:483 | universal_protocol_translation_spec.md:effort table (thinking caps)
    expect(spec).toBeDefined()
  })
  ...
  it('relay vs direct tier: provider relay flag coherent', () => {
    const prov = PROVIDERS['alibaba']
    // relay=false passthrough=true citation: reference/OmniRoute/open-sse/config/providers/registry/alibaba/index.ts:4 | reference/OmniRoute/src/shared/constants/modelSpecs.ts:483
